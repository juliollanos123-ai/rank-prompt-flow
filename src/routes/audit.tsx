...
function AuditForm({ tier, onSubmit }: { tier: Tier; onSubmit: () => void }) {
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
        // TODO: wire to Lovable Cloud / Airtable / N8N when backend is enabled
        onSubmit();
      }}
    >
      ...
      <button
        type="submit"
        className="group mt-10 inline-flex items-center gap-3 rounded-full bg-prompt px-7 py-4 font-display text-sm uppercase tracking-wider text-primary-foreground transition-all hover:shadow-elegant"
      >
        {tier === "free" ? "Get my free diagnosis" : "Request full audit"}
